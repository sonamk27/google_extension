...
function valueLooksLikePII(value) {
  if (globalThis.PrivacyRules?.valueLooksLikePII) {
    return globalThis.PrivacyRules.valueLooksLikePII(value);
  }
...
function isSensitiveField(el) {
  ...
  if (globalThis.PrivacyRules?.isSensitiveFieldAttributes) {
    return globalThis.PrivacyRules.isSensitiveFieldAttributes({
      type,
      autocomplete,
      name,
      id,
...
function extractDomStructure(maxElements = 150) {
  ...
  for (const el of nodes) {
    ...
    elements.push({
      ...
      text: sensitive ? null : rawText.trim().slice(0, 100),
      bbox: {
        x: Math.round(rect.x),
        y: Math.round(rect.y),
...
function executeAction(action) {
  try {
    ...
    if (action.type === "click") {
      ...
      if (!el) return { ok: false, error: `target element "${action.targetId}" not found` };
    ...
    if (action.type === "type") {
      ...
      // Robust setter for React/Vue/Angular controlled inputs
      const prototype =
      ...
      // Dispatch standard input and change events with bubble support
    ...
    if (action.type === "none") {
      return { ok: true, note: "task complete or no action required" };
    }
